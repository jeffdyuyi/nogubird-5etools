...
import {
    ...
    PANEL_TYP_ERROR,
    PANEL_TYP_BLANK,
} from "./playerscreen/playerscreen-consts.js";
...
} from "./playerscreen/playerscreen-panels.js";
...
class Board {
    constructor() {
        ...
        this._pDoSaveStateDebounced = MiscUtil.debounce(() => StorageUtil["pSet"]("playerscreen", this.getSaveableState()), 25);
    }
    ...
    async _pLoadTempData() {
        const temp = await StorageUtil["pGet"]("playerscreen_temp_sublist");
        if (!temp) return;

        try {
            await this._pLoadTempData_({ temp });
        } finally {
            await StorageUtil["pRemove"]("playerscreen_temp_sublist");
        }
    }
    ...
    async pHasSavedState() {
        return !!await StorageUtil["pGet"]("playerscreen");
    }
    ...
    async pDoLoadState() {
        let toLoad;
        try {
            toLoad = await StorageUtil["pGet"]("playerscreen");
        } catch (e) {
            JqueryUtil.doToast({
                content: `Error when loading DM screen! Purged saved data. ${VeCt.STR_SEE_CONSOLE}`,
                type: "danger",
            });
            await StorageUtil["pRemove"]("playerscreen");
            setTimeout(() => { throw e; });
            return;
        }
    ...
    async _pDoLoadState_pHandleError({ toLoad, e }) {
        ...
        const handleClickDownload = () => {
            DataUtil["userDownload"](`dm-screen`, toLoad, /** @type {any} */({ fileType: "dm-screen" }));
        };
        ...
        const handleClickPurge = async () => {
            if (!await InputUiUtil["pGetUserBoolean"](/** @type {any} */({ title: "Purge", htmlDescription: "你确定吗？", textYes: "是的", textNo: "取消" }))) return;
            await StorageUtil["pRemove"]("playerscreen");
            doClose(true);
        };
...
class SideMenu {
    ...
    render() {
        ...
        const wrpResizeW = ee`<div class="ve-w-100 ve-mb-2 ve-split-v-center"><div class="sidemenu__row__label">宽</div></div>`.appendTo(this.eleMnu);
        ...
        const wrpResizeH = ee`<div class="ve-w-100 ve-mb-2 ve-split-v-center"><div class="sidemenu__row__label">高</div></div>`.appendTo(this.eleMnu);
        ...
        const btnSetDim = ee`<button class="ve-btn ve-btn-primary" style="width: 100%;">设置尺寸</div>`.appendTo(wrpSetDim);
        btnSetDim.onn("click", async () => {
            const w = Number(iptWidth.val());
            const h = Number(iptHeight.val());

            if (w > 10 || h > 10) {
                if (!await InputUiUtil["pGetUserBoolean"](/** @type {any} */({ title: "Too Many Panels", htmlDescription: "That's a lot of panels. 你确定吗？", textYes: "是的", textNo: "取消" }))) return;
            }
        ...
        const btnSaveFile = ee`<button class="ve-btn ve-btn-primary">保存到文件</button>`.appendTo(wrpSaveLoadFile);
        btnSaveFile.onn("click", () => {
            DataUtil["userDownload"](`dm-screen`, this.board.getSaveableState(), /** @type {any} */({ fileType: "dm-screen" }));
        });
        const btnLoadFile = ee`<button class="ve-btn ve-btn-primary">从文件加载</button>`.appendTo(wrpSaveLoadFile);
        btnLoadFile.onn("click", async () => {
            const { jsons, errors } = await InputUiUtil["pGetUserUploadJson"](/** @type {any} */({ expectedFileTypes: ["dm-screen"] }));
        ...
        const btnSaveLink = ee`<button class="ve-btn ve-btn-primary">保存为URL</button>`.appendTo(wrpSaveLoadUrl);
        ...
        const wrpCbConfirm = ee`<div class="ve-w-100 ve-split-v-center"><label class="sidemenu__row__label sidemenu__row__label--cb-label"><span>关闭时保存</span></label></div>`.appendTo(this.eleMnu);
    ...
    doUpdateHistory() {
        ...
        if (this.board.exiledPanels.length) {
            const wrpHistHeader = ee`<div class="ve-w-100 ve-mb-2 ve-split-v-center"><span style="font-variant: ve-small-caps;">近期删除</span></div>`.appendTo(this.wrpHistory);
...
class Panel {
    ...
    async pDoMassPopulate_Entities(evt) {
        ...
        const pFnConfirmPanels = () => InputUiUtil["pGetUserBoolean"](/** @type {any} */({ title: "Add as Panels", htmlDescription: "Adding entries one-per-panel may resize your DM Screen<br>Are you sure you want to add as panels?", textYes: "Yes", textNo: "Cancel" }));

        await ListUtilEntity["pDoUserInputLoadSublist"](/** @type {any} */({
            page,
            ...
            optsSaveManager: {}, // Added dummy to appease TS
        }));
    }
    ...
    _getBtnSelTab(ix, title) {
        ...
        const doCloseTabWithConfirmation = async () => {
            if (this.board.getConfirmTabClose()) {
                if (!await InputUiUtil["pGetUserBoolean"](/** @type {any} */({ title: "Close Tab", htmlDescription: `Are you sure you want to close tab "${this.tabDatas[ix].title}"?`, textYes: "Yes", textNo: "Cancel" }))) return;
            }
            this.doCloseTab(ix);
        };
...
class AddMenu {
    ...
    async pRender() {
        ...
        await Promise.all(this.tabs.map(t => t.pRender()));
        ...
        if (!this.eleTab) {
            ...
            const iptFile = ee`<input type="file" class="hidden">`
                .onn("change", (evt) => {
                    ...
                    reader.onload = async () => {
                        const base64 = String(reader.result).replace(/.*,/, "");
...
class AddMenuSearchTab extends AddMenuTab {
     ...
     * @param indexes
     * @param subType
     * @param adventureOrBookIdToSource
     * @param opts
     */
    constructor({ indexes, subType = "content", adventureOrBookIdToSource = null, ...opts }) {
        super({ board: opts.board, ...opts, label: AddMenuSearchTab._getTitle(subType) });
...
class AdventureOrBookView {
    ...
    _render({ isSkipMissingData = false } = {}) {
        ...
        const stack = [];
        const page = AdventureOrBookView._PROP_TO_URL[this._prop];
        Renderer
            .get()
...
window.addEventListener("load", () => {
    ...
    window["PLAYER_SCREEN"] = new Board();
    Renderer.hover.bindDmScreen(window["PLAYER_SCREEN"]);
    window["PLAYER_SCREEN"].pInitialise()
        .catch(err => {
